  ...
  for (struct FieldInfo * f = ci->fields; f->name; f++) {
    if (f->width == 1) {
      fprintf(stderr, "  { B@%d. %s=$%02x=%d. }\n",
              f->offset, f->name, BF(w, f->offset), BF(w, f->offset));
      ...
      if (!strcmp("B_cls", f->name) || !strcmp("B_owner", f->name)) {
        word num = BF(w, f->offset);
        assert(num);
      ...
      fprintf(stderr, "  { W@%d. %s=$%02x=%d. }\n",
              f->offset, f->name, WF(w, f->offset), WF(w, f->offset));
    } else {
      assert(0);
    }
  }
